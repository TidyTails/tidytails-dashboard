...
- **Mar 24:** Cron fix saga RESOLVED (v4). Weekly Review root cause: **delivery config was missing `to` chatId** — literally didn't have the target. EOD cron had correct config but agent sessions were calling message tool themselves. Fixes: (1) Added `to: "-1003877675524"` to Weekly Review delivery, (2) v3 prompt removes all Telegram mentions. Test run triggered 2 AM.
...
15. **Cron Telegram delivery — TWO failure modes:** (a) Missing `to` chatId in delivery config = guaranteed error "Delivering to Telegram requires target <chatId>". ALWAYS set `{"mode": "announce", "channel": "telegram", "to": "-1003877675524"}`. (b) Agent sessions calling the message tool themselves = intermittent errors. Fix: don't mention Telegram in cron prompts AT ALL — the word triggers agents to try sending.
