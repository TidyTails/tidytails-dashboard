# Competitive Intelligence - St. Louis Pet Waste Removal

## Last Updated: 2026-03-15 @ 2:04 AM (Competitor Pulse Check)

### Latest Update Summary (Mar 15, 2 AM)
- 🔴 NEW: Dog Doody Defenders — Chesterfield/Florissant, $17/visit weekly ($68/mo), 32% cheaper than TJ
- Scoop Soldiers STL — CONFIRMED BACK with full SEO landing page (2000+ words)
- 🆕 Dirty Dog Wash Poo Crew — $95/mo weekly, add-on service from dog wash biz
- Pawfect Waste Solutions — Still DEAD (404)
- Fresh Paws — Still DEAD (DNS failure)
- TJ search visibility — STILL ZERO for brand name searches
- TJ is now the MOST EXPENSIVE tracked competitor in market ($100/mo weekly)
- Full pulse check: `memory/2026-03-15-competitor-pulse-check.md`

**Previous updates:**
- Mar 13 3PM: Scoop Soldiers back, Poop 2 Scoop $18/visit, Doggy Doods discovered
- Mar 13 battle card: `dashboard/research/2026-03-13-COMPETITIVE-BATTLE-CARD.html`

---

## 🚨 MARKET UPDATE: COMPETITOR STATUS CHANGED

| Competitor | Previous Status | Current Status (Mar 13) |
|------------|--------|----------|
| **Dog Doody Defenders** | Not tracked | 🔴 NEW — Chesterfield + Florissant, $17/visit, website + FB + MapQuest + Yelp |
| **Dirty Dog Wash Poo Crew** | Not tracked | 🟡 NEW — $95/mo weekly, add-on from dog wash business |
| **Pawfect Waste Solutions** | 404 - DEAD | Still 404 ✅ Dead |
| **Scoop Soldiers STL** | 404 → BACK (Mar 13) | ✅ CONFIRMED BACK — full SEO page, heavy content, no pricing listed |
| **Fresh Paws** | Squarespace EXPIRED | Still dead (DNS not resolving) ✅ |
| **Doggy Doods** | Unknown | 🆕 St. Charles County. Eugene. Great reviews. In TJ's expansion zone |
| **Little Stinkers of MO** | Known | ⚠️ GROWING — O'Fallon city pages, deodorizing + litter box + waste stations |

**THE SITUATION (Mar 15):**
Market is getting MORE crowded, not less. 3 new/returning competitors in 2 weeks. TJ is the most expensive option AND the least visible. Dog Doody Defenders is in Chesterfield (TJ's home turf) at 32% less. Spring season = window narrowing fast.

---

## ⚠️ ACTIVE COMPETITOR: Poop 2 Scoop

**Website:** poop2scoop.com (LIVE, professional)
**Phone:** 314-449-9447
**Type:** Local family business (direct competitor)

### Pricing Comparison (IMPORTANT)
| Service | Poop 2 Scoop | Tidy Tails | Gap |
|---------|--------------|------------|-----|
| Weekly (1 dog) | **$18** | $25 | -28% cheaper |
| Every other week | **$25** | $45 | -44% cheaper |
| Extra dog | $4 | ? | -- |
| Large yard | $4 | ? | -- |

### What They Offer
- Residential & Commercial
- No contracts (same as TJ)
- Deck/yard deodorizing
- 100% satisfaction guarantee
- Family-run, local

### How TJ Can Compete
1. **Personal touch** — TJ is the owner, P2S is a crew
2. **Reliability** — Emphasize never missed, always on time
3. **Commercial focus** — P2S lists commercial but may not pursue it
4. **Price justification** — $7/week more = premium service

---

---

## ⚠️ COMPETITOR UPDATE: Scoop Soldiers

**Status:** STL PAGE NOW 404 — Possible pullback or restructuring
**Type:** National franchise
**Website:** scoopsoldiers.com/locations/st-louis → **NOW 404**
**Main Site:** scoopsoldiers.com — Still active nationally

### 🔄 STATUS CHANGE (Feb 22, 2026 - 4 PM)
- **1:00 PM:** STL landing page was LIVE
- **4:00 PM:** STL landing page returns 404 ("This Stinks!")
- **Locations Page:** Generic, no STL-specific listing visible
- **St. Louis Google Reviews:** Still none

**WHAT THIS MEANS:**
- Either they pulled out of STL launch
- Or technical issue / URL restructure
- Either way: THREAT LEVEL REDUCED (for now)
- Still claim GBP — position is vulnerable regardless

### Why GBP Still Matters
Even if Scoop Soldiers paused STL, the market is wide open:
- Pawfect Waste Solutions (pooperscooperstl.com) = 404 (DEAD)
- No dominant local player with strong Google presence
- First to claim GBP + get reviews wins the market

### What They Are
- Founded 2019, rapid national expansion
- 8 new markets in 2026 (including St. Louis)
- 1,300+ reviews at 5.0 stars
- Franchise model (well-funded)
- 877-930-7667 toll-free

### Their Pricing vs Tidy Tails
| Service | Scoop Soldiers | Tidy Tails | Gap |
|---------|---------------|------------|-----|
| Weekly | **$15/visit** | $25/week | -40% |
| Biweekly | ~$20-25 est | $45 | -44% to -55% |
| Monthly | ~$50-60 est | $70 | -14% to -28% |

### Their Competitive Advantages
- FREE first cleanup (no risk trial)
- 100% money-back guarantee
- No contracts required
- 24/7 customer support team
- Uniformed, background-checked techs
- Professional systems (reminders, photos, gate closure confirmation)
- Commercial focus (HOAs, property managers, landlords)
- Deodorizer + sanitization upsells
- Pet waste station installation/maintenance

### Their Weaknesses (Tidy Tails Opportunities)
- Corporate/franchise feel (not local)
- Rotating technicians (no personal relationship)
- National pricing may not account for local dynamics
- Brand new to STL (no local reputation yet)
- Heavy military branding may not resonate with everyone

### Tidy Tails Counter-Strategy
1. **Double down on LOCAL** - "Your neighbor, not a franchise"
2. **Personal relationship** - Same person every time, knows your dogs
3. **Speed to reviews** - Get Google reviews NOW before they build STL presence
4. **Commercial first** - Beat them to property managers while they're new
5. **Price positioning** - Don't compete on price, compete on trust

---

## 💡 OPPORTUNITY: PooPrints DNA Partnership

**Website:** pooprints.com
**What It Is:** DNA testing to identify dog waste violators in apartment complexes

### How It Works
1. Property requires all dogs to register DNA
2. When waste found, sample sent to PooPrints
3. Violator identified with 100% accuracy
4. $200+ fines issued to offender
5. 95% waste reduction after implementation

### Partnership Opportunity for Tidy Tails
- PooPrints needs LOCAL partners to do the actual cleanup
- Could position as "PooPrints Implementation Partner" for STL
- Target: Apartment complexes, HOAs, dog parks
- Recurring revenue: Regular cleanup + sample collection

### Key Stats (from NY Times 2026-02-20)
- 1.2 million dogs registered
- 8,000 localities using service
- Growing rapidly in urban areas

### Next Step
Contact PooPrints about becoming a service partner in St. Louis area.

---

## 📊 Market Intelligence

### Local Competitors (Need Further Research)
- Pawfect Scoops - Status unknown, may be inactive
- DoodyCalls - National franchise, check STL presence
- Pet Butler - National franchise, check STL presence

### Market Dynamics
- Winter churn is normal in industry
- Spring = peak acquisition season (NOW)
- Commercial accounts more stable than residential
- HOAs increasingly mandating pet waste services

---

## 🎯 Strategic Implications

### Immediate Actions (This Week)
1. **Claim Google Business Profile** - Critical before Scoop Soldiers builds STL reviews
2. **Get 3-5 reviews** - Need social proof NOW
3. **Hit commercial accounts hard** - They're new, we can beat them there
4. **Research PooPrints partnership** - Could be differentiation play

### Positioning Pivot
**Old:** Generic pet waste removal service
**New:** "Your local dog waste expert - same person every time, not a franchise"

### Pricing Strategy
- DON'T drop prices to match Scoop Soldiers
- They're racing to the bottom on price
- We differentiate on relationship, trust, local
- Consider: "Local premium" positioning
- Add upsells: Deodorizing, one-time deep cleans

---

## Intelligence Sources
- scoopsoldiers.com (direct research)
- einpresswire.com (press release Feb 10, 2026)
- pooprints.com (partnership research)
- nytimes.com (Feb 20, 2026 - DNA testing article)
- rentalhousingjournal.com (apartment complex trends)

---

*Next intelligence update: Research DoodyCalls and Pet Butler STL presence*
